      ...
      <form action="{{ route('"'"'ajustes-razonables.update'"'"', $ajuste) }}" method="POST">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <input type="date" id="fecha_solicitud" name="fecha_solicitud" value="{{ old('"'"'fecha_solicitud'"'"', $ajuste->fecha_solicitud?->format('"'"'Y-m-d'"'"')) }}" class="form-control @error('"'"'fecha_solicitud'"'"') is-invalid @enderror" required>
            @error('"'"'fecha_solicitud'"'"')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>
          <div class="col-md-4">
            <label for="fecha_inicio" class="form-label">Fecha de inicio</label>
            <input type="date" id="fecha_inicio" name="fecha_inicio" value="{{ old('"'"'fecha_inicio'"'"', $ajuste->fecha_inicio?->format('"'"'Y-m-d'"'"')) }}" class="form-control @error('"'"'fecha_inicio'"'"') is-invalid @enderror">
            @error('"'"'fecha_inicio'"'"')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>
          <div class="col-md-4">
            <label for="fecha_termino" class="form-label">Fecha de término</label>
            <input type="date" id="fecha_termino" name="fecha_termino" value="{{ old('"'"'fecha_termino'"'"', $ajuste->fecha_termino?->format('"'"'Y-m-d'"'"')) }}" class="form-control @error('"'"'fecha_termino'"'"') is-invalid @enderror">
