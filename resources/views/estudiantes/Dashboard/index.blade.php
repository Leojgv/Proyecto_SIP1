...
@php
    $coleccionNotificaciones = isset($notificaciones) ? $notificaciones : collect();
@endphp
...
<div class="container-fluid">
  ...
  <div class="row g-4 mt-5 align-items-start" id="notificaciones">
        ...
        <div class="card-body">
          <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
              ...
              <p class="text-muted mb-0">Revisa los avisos y actualizaciones más recientes.</p>
            </div>
            @if (($notificacionesSinLeer ?? 0) > 0)
              <span class="badge bg-primary-subtle text-primary">{{ $notificacionesSinLeer }} nuevas</span>
            @endif
          </div>
          ...
          <div class="list-group list-group-flush">
            @forelse ($coleccionNotificaciones as $notification)
              @php
                  $payload = is_array($notification->data) ? $notification->data : (array) $notification->data;
                  $titulo = $payload['titulo'] ?? $payload['title'] ?? 'Actualización en tu cuenta';
                ...
                <div class="d-flex align-items-start justify-content-between gap-3">
                  ...
                  <div class="text-end">
                    @if (is_null($notification->read_at))
                      <span class="badge bg-light text-primary border">Nueva</span>
                    @endif
                    @if ($enlace)
                      <div>
            ...
            @empty
              <p class="text-muted text-center my-4">Aún no tienes notificaciones pendientes.</p>
            @endforelse
          </div>
        </div>
      </div>
    ...
    <div class="col-12 col-xl-6" id="configuracion">
      <div class="card border-0 shadow-sm">
        <div class="card-body">
          <h5 class="card-title mb-1">Configuración</h5>
          <p class="text-muted">Mantén tus datos de contacto al día para recibir avisos oportunamente.</p>
